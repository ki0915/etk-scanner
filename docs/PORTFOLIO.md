...
비용 인식 LLM 보안 스캐너의 설계, 구동, 실증 기록.
취약점 발견에서 끝내지 않고 수정한 뒤 실제 서버에서 막혔는지까지 확인한다.
...
| 항목 | 내용 |
|------|------|
| 개요 | 예산 상한 안에서 취약점을 찾고 수정하고 실증하는 LLM 보안 스캐너 |
| 차별점 | 비용을 1급 지표로 — 정적 무료 티어 + 저가/고가 모델 계층화 + 비용 회계 |
| 개발 환경 | Claude Code (Anthropic API) |
| 실증 | TS/Express 백엔드 감사 → 하드코딩 시크릿 발견 → HTTP exploit before/after |
| 비용 | 멀티턴 방식 대비 6.7× 절감 (5,000원 → 750원, 동일 대상 자체 측정) |
...
개발 과정에서 부딪힌 문제와 그에 대한 판단을 기록한다.

| 문제 | 판단 | 근거 |
|------|------|------|
| 멀티턴 방식의 비용 폭증 | 단일턴 캐스케이드로 전환 | 동일 대상 6.7× 절감 측정 |
| 싱크 기반이 로직버그를 놓침 | 보안 의도 함수 탐지로 전환 | SSRF류 로직버그 탐지 |
| "재현됨"을 취약점으로 오판 | 보안영향 필터 추가 | 체크섬 미검증 같은 기능버그 자동 제외 |
| 이미 알려진 취약점 신고 위험 | OSV 중복 체크 도입 | 신고 전 자동 조회 |
| 실증 중 포트 잔존으로 오판 | 즉시 정정 후 재측정 | 200→401 재검증 |
...
**단기**
- OSV/GHSA 중복 체크를 스캔 흐름에 연결해 기지 취약점을 신고 전 자동 제외
- Docker 격리 환경에서 e2e exploit 검증을 CI 단계로 자동화
- 비용 벤치마크를 여러 대상에 돌려 단계별 수치를 공개

**중기**
- 정적 탐지 다언어 확대 (Go, Java)
- tree-sitter 도입으로 JS/TS 탐지 오탐 축소
- 과거 CVE 패치 diff를 분류 단계 few-shot으로 활용

**장기**
- 라이브러리 취약점과 실제 사용 코드베이스 연결 → 책임 소재·실제 악용 지점 추적
- 조직 단위 피드백 메모리로 반복 오탐 억제
