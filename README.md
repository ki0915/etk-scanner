...
### 벤치마크 (SecurityEval, 121개 레이블 취약 코드)

레이블된 취약점 데이터셋에 LLM 티어(Haiku)를 돌린 실측:

| 지표 | 값 |
|------|:--:|
| 재현율 (Recall) | 91.7% (111/121) |
| CWE 정확도 (탐지 중 유형 일치) | 37.8% |
| 총 비용 | **67원** (121개, 28K 토큰) |
| 비용/샘플 | **0.55원** |

> 정직한 한계: SecurityEval은 안전 샘플이 없어 **정밀도(오탐)는 미측정**.
> 재현율만으로는 "무조건 취약" 판정도 높게 나오므로, 유형까지 맞춘
> **CWE 정확도(37.8%)가 더 의미 있는 지표**다. 재현: `python scripts/bench_eval.py`
